...
public class DeepLearnerHudRenderer implements IGuiOverlay {
	...
	public void render(ForgeGui gui, PoseStack poseStack, float partialTick, int screenWidth, int screenHeight) {
		...
		GuiComponent.blit(poseStack, 3, 3, 0, 23, 113, 1, 256, 256);
		for (int i = 0; i < renderable.size(); i++) {
			GuiComponent.blit(poseStack, 3, 4 + spacing * i, 0, 24, 113, spacing, 256, 256);
			CachedModel cModel = renderable.get(i).getLeft();
			GuiComponent.blit(poseStack, x + 18, y + i * spacing + 10, 0, 0, 89, 12, 256, 256);
			int width = 87;
			...
			GuiComponent.blit(poseStack, x + 19, y + i * spacing + 11, 0, 12, width, 10, 256, 256);
		}
		GuiComponent.blit(poseStack, 3, 4 + spacing * renderable.size(), 0, 122, 113, 2, 256, 256);
		...
		for (int i = 0; i < renderable.size(); i++) {
			...
			mc.font.drawShadow(poseStack, comp, x + 4, y + spacing * i, 0xFFFFFF);
			mc.font.drawShadow(poseStack, Component.translatable("hostilenetworks.hud.model"), x + mc.font.width(comp) + 4, y + spacing * i, 0xFFFFFF);
			if (cModel.getTier() != ModelTier.SELF_AWARE) mc.font.drawShadow(poseStack, I18n.get("hostilenetworks.hud.kills", cModel.getKillsNeeded()), x + 21, y + 12 + i * spacing, 0xFFFFFF);
		}
	}
	...
	public static void drawModel(Minecraft mc, int x, int y, ItemStack stack, CachedModel model) {
		...
		mc.font.drawShadow(matrix, Component.translatable("hostilenetworks.hud.model"), x + mc.font.width(comp) + 4, y, 0xFFFFFF);
		RenderSystem.setShader(GameRenderer::getPositionTexShader);
		RenderSystem.setShaderColor(1.0F, 1.0F, 1.0F, 1.0F);
		RenderSystem.setShaderTexture(0, DL_HUD);
