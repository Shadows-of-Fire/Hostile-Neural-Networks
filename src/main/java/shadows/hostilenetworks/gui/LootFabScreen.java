...
public class LootFabScreen extends ContainerScreen<LootFabContainer> {
	...
	public void render(MatrixStack stack, int pMouseX, int pMouseY, float pPartialTicks) {
		...
		if (this.model != null) {
			...
			this.buttons.get(1).visible = this.currentPage < this.model.getFabDrops().size() / 9;
		} else {
	...
	protected void renderTooltip(MatrixStack pPoseStack, int pX, int pY) {
		...
		if (selection != -1 && this.isHovering(79, 5, 16, 16, pX, pY)) {
			this.renderWrappedToolTip(pPoseStack, Arrays.asList(new TranslationTextComponent("hostilenetworks.gui.clear")), pX, pY, this.font);
	...
	protected void renderBg(MatrixStack stack, float pPartialTicks, int pX, int pY) {
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					if (y * 3 + x < Math.min(drops.size() - this.currentPage * 9, 9) && this.isHovering(18 + 18 * x, 10 + 18 * y, 16, 16, pX, pY)) {
						AbstractGui.blit(stack, left + 16 + 19 * x, top + 8 + 19 * y, 13, 83, 18, 18, 256, 256);
					}
				}
			}
		}
		...
		if (selection != -1 && selection / 9 == this.currentPage) {
			...
			AbstractGui.blit(stack, left + 16 + 19 * (selIdx % 3), top + 8 + 19 * (selIdx / 3), 31, 83, 18, 18, 256, 256);
		}

		mc.getTextureManager().bind(PLAYER);
		AbstractGui.blit(stack, left, top + 88, 0, 0, 176, 90, 256, 256);
